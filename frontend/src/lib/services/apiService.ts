...
function mapBackendToFrontend(backendData: any): Model3D {
	...
	return {
		...
		metadata: {
			...
			updatedAt: new Date(),
			generatedFrom: 'text',
			volume: backendData.metadata?.volume || 0 
		}
	};
}
...
class ApiService {
	...
	async exportModel(modelId: string, exportFormat: ExportFormat, modelObj?: Model3D): Promise<Blob> {
 		// Prepare body; include full model if provided so backend can export even if it lost in-memory state
 		const body: any = {
 			model_id: modelId,
 		...
 		if (modelObj) {
 			body.model = {
 				id: modelObj.id,
 				name: modelObj.name,
 				parameters: mapFrontendToBackend(modelObj.parameters),
 				geometry: {
 					vertices: Array.from(modelObj.geometry.vertices),
 					indices: Array.from(modelObj.geometry.indices),
 					normals: modelObj.geometry.normals ? Array.from(modelObj.geometry.normals) : undefined
 				},
 				metadata: {
 					created_at: new Date(modelObj.metadata.createdAt).toISOString(),
 					updated_at: new Date(modelObj.metadata.updatedAt).toISOString(),
 					generated_from: modelObj.metadata.generatedFrom || 'text',
 					source_prompt: modelObj.metadata.sourcePrompt || null,
 					volume: modelObj.metadata.volume || 0
 				}
 			};
 		}
