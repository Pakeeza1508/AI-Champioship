...
import { MATERIALS } from '$lib/data/materials'; // Import materials

export type ComponentType = 'wings' | 'fuselage' | 'engines';

export interface ComponentMaterialState {
    wings: string;
    fuselage: string;
    engines: string;
}

export const componentMaterials = writable<ComponentMaterialState>({
    wings: 'al7075',
    fuselage: 'al2024',
    engines: 'ti6al4v'
});

// NEW: Store for Mission/Physics Context
export const missionProfile = writable({
