...
async def generate_from_text(request: GenerateRequest):
    ...
    """
    try:
        # Extract parameters from text using AI (measure time)
        t0 = time.time()
        parameters = await ai_service.extract_parameters_from_text(request.prompt)
        t1 = time.time()
        print(f"[GEN] extract_parameters_from_text took {t1 - t0:.2f}s", flush=True)

        # Generate 3D model from parameters (measure time)
        t0 = time.time()
        ...
        t1 = time.time()
        print(f"[GEN] create_model_from_parameters took {t1 - t0:.2f}s", flush=True)
